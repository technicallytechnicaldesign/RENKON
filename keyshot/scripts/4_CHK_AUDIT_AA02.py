...
# AUTHOR ttd
# REV AA02
# Standalone Python 3 script -- NOT a KeyShot script, run with a regular
...
#      convention, reporting exactly which component is wrong rather than a
#      generic pass/fail.
...
#
# Usage:
#   python 4_CHK_AUDIT_AA02.py /path/to/scripts_or_output_folder
# or run with no arguments and it will ask.
...
def collectPyFiles(rootFolder):
    ...
    literally named 'archive' (case-insensitive) -- archived scripts are
    pre-convention on purpose."""
    found = []
...
# order -- manifests observed in this pipeline use "output_path", but this
# list keeps the check tolerant of minor schema drift between scripts.
OUTPUT_PATH_COLUMNS = ("output_path", "outputPath", "output_file", "path")

# Heuristic classifier only -- built from status strings observed across this
# pipeline's manifests (render_manifest.csv, reveal_manifest.csv,
# cutaway_manifest.csv, material_lookup_manifest.csv). New scripts may
# introduce status strings this won't recognize; those fall into the
# "unclassified" bucket below rather than being silently miscounted as good
# or bad.
