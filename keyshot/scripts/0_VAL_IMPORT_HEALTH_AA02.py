...
# REV AA02
# HEADLESS COMPLIANT
# Trial-import geometry/appearance health check: cheap, fast pre-flight gate
# to run before committing a folder to the expensive multi-Studio batch
# render pass. For every file of a chosen extension in a folder: trial
# lux.newScene() + lux.importFile() plus three lightweight checks, no
# rendering.
#
#   1. IMPORT FAILURE -- caught and reported per-file so one bad file does
#      not crash the whole folder run.
#   2. BOUNDING-BOX SANITY -- near-zero volume flags a degenerate/failed
#      import (geometry silently missing even though importFile() did not
#      raise); suspiciously large bbox is a softer heuristic flag. Targets
#      choppy-tessellation-around-chamfers failure modes.
#   3. OBJECT/MESH COUNT SANITY -- an assembly-named file (filename contains
#      ASSY/ASSM/ASM, case-insensitive) resolving to only 1 object node is a
#      possible missing-components symptom. 0 object nodes = empty import.
#   4. STUDIO/CAMERA PRESENCE -- a part with neither Studios nor raw cameras
#      falls back to raw-camera rendering with whatever lighting is active;
#      this surfaces that for a whole folder before a batch run starts.
#
# API facts: lux.newScene/importFile/getSceneTree/isHeadless/getInputDialog,
# node.isObject/getChildren/getName, lux.getStudios/getStudio/getCameras are
# confirmed calls. SceneNode.getBoundingBox() is confirmed to exist and to
# return (min, max) vectors; exact return shape (tuple vs .min/.max object,
# world-space kwarg name) is not confirmed, so resolve_bounding_box() /
# _parse_bbox_result() probe a short candidate list of names/shapes (same
# defensive pattern used elsewhere in this pipeline).
#
# Heuristic thresholds (SUSPICIOUS_SIZE_THRESHOLD, ASSEMBLY_NAME_HINTS) are
# this script's own tunable judgment calls, not API facts -- treat every
# FLAG in the manifest as worth a look, not a certainty.
import os, re, sys
import os.path
import csv
...
def collectObjectNodes(node, results=None):
    ...
    # 1_HLP_MAT_LOOKUP_AA02.py -- keeps node references so callers can count
...
# and _parse_bbox_result() in 2b_ANI_CUTAWAY_REVEAL_AA02.py and
# 2b_ANI_ASSEMBLY_PROCEDURAL_AA02.py. See CONFIRMED vs EXPERIMENTAL header
