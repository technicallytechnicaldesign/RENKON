...
================================================
Assembly's own parts converge into their authored final transforms; camera
static. Parts-move counterpart to HERO_REVEAL (camera moves) and
CUTAWAY_REVEAL (clip plane sweeps).

Modes: scatter_settle (random offset/rotation settles home in a straight
line), staggered_build (parts arrive one at a time in order), spiral_converge
(scatter_settle path plus a decaying perpendicular spiral), ghost_fade
(parts stay put, fade in from transparent).

TRANSFORM MECHANICS: no confirmed absolute get/set-position API for a scene
node exists; the only confirmed per-node mutator is
node.applyTransform(matrix, absolute=False). Motion is a per-part
displacement function of progress t in [0,1] with displacement(t=1)=0; each
frame applies only the delta displacement(t_now)-displacement(t_prev), so
deltas telescope to net zero over the shot by construction, not readback
(no confirmed getPosition/getRotation exists to verify). Rotation deltas
are single-fixed-axis only (angle-additive, safe to sum -- no multi-axis
composition). A corrective delta (negative of the running applied total)
is issued per part on the final frame against float drift; this checks
this script's own bookkeeping, not a verified readback.
luxmath.Matrix().translate(vector) has no prior confirmed use in this
pipeline and is probed via getattr, degraded gracefully if absent -- every
translation-based mode depends on it.

PART EXTENT: scatter/spiral offsets scale with each part's own size.
SceneNode.getBoundingBox() is confirmed; resolve_bounding_box() probes a
short candidate list since the exact return shape isn't documented, and
falls back to center-to-origin distance if nothing resolves.

GHOST FADE: no confirmed opacity/material API exists anywhere in this
pipeline. Two layers are probed via getattr: node-level setOpacity/
setTransparency/setAlpha, then material-object accessors (getMaterialNode/
getAppearance/getMaterialObject/getMaterialGraph -- not node.getMaterial(),
which returns a name string, not a settable object) with the same setters.
No working API means a hard cut (part visible for the whole shot) instead
of a silent no-op fade.

SUB-ASSEMBLY BUILD (grouped convergence) is a known extension point, not
implemented here -- see get_parts()/schedule_staggered_windows().

Confirmed on this pipeline's KeyShot build: lux.getActiveEnvironment(),
env.getBrightness/setBrightness, env.setBackplateImage, lux.renderImage,
lux.encodeVideo, lux.openFile(), lux.getStudios/getStudio/setActiveStudio,
lux.getCameras/setCamera, lux.getModelSets/setModelSets,
lux.getImportOptions, lux.importFile, lux.getSceneTree, root.find(),
node.isObject(), node.getName(), SceneNode.getCenter(world=True),
env ground-shadow/reflection getters and setters, lux.getRenderOptions/
setAddToQueue/setMaxTimeRendering, lux.processQueue, lux.isHeadless,
lux.getInputDialog. env.setRotation() is inferred by naming-convention
analogy, unconfirmed.
...
def get_target_node(model_set_name):
    ...
    set if given and found, else the whole scene root."""
...
def set_ground_rendering(env, enabled):
    ...
    are confirmed; setter names are inferred by naming-convention analogy
    and tried defensively."""
    if env is None:
        return False
...
def resolve_camera_list(raw_value):
    ...
    if v.upper() == "ALL":
        try:
            ...
            studios = []
        covered = set()
        # getStudio is 2024.1+ only; getattr-guard so listing stays
        # version-safe on 11.0 builds (covered stays empty there).
...
# --------------------------------------------------------------------------
# EXPERIMENTAL: bounding-box probe
# --------------------------------------------------------------------------
...
def encode_one_assembly(result, opts, output_folder):
    ...
    exists on disk -- queued shots defer this until the queue is
    processed (a video can't be encoded from a still-incomplete folder)."""
