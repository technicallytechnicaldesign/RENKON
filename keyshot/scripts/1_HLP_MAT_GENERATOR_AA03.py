...
# makes the whole script fail to load in the Scripting Console.
#
...
# mappable. See MWR-9C4E21 (MASKED_WEAR_RESEARCH, private RENKON_LAB repo).
...
A variant generator: a dialog full of tick-box feature layers, each
independently toggleable, combined onto a chosen base material
(metal/plastic) at a chosen wear level. Every generated material gets a
random, unique, technical-style name (e.g. MAT-ALU-LGT-7F3A2C) rather than
...
# Subtle base amplitudes at Light Wear (1.0x) -- these read as much stronger
# visually than the raw [0,1] numbers suggest, so they are kept low.
BASE = {
...
# NOTE: Thin Film is not offered as a layer. It is a full KeyShot *material
# type* (its own iridescent BRDF, like Metal or Plastic), not a texture with
# a bump/height output -- wiring it into a bump input slot is a category
# error with undefined behavior. Offering it properly would mean an
# alternate base material (a MATERIALS row), not a layer toggle.
FEATURE_KEYS = [
...
def get_options():
    ...
    def norm_item(v, valid):
        ...
        # something that isn't a real option, or an int index bypasses the
        # selection and silently falls through to the default material/wear.
...
def add_color_gradient(graph, base_node, base_color):
    ...
    # possibly high-contrast) in place. Best-effort: this node's UI is a
    # draggable color bar, which may not expose simple named color parameters
    # the way other nodes do. If no match is found, that's reported rather
    # than assumed.
    ...
    if not (ok1 or ok2):
        ...
        # colour-stop params, so it can't be set from script -- it keeps its
        # default (magenta->cyan) stops. Do NOT wire that into the base
        # colour, since an unset gradient drives a garbage (magenta) result.
        # Leave the node unwired (harmless) and keep the base colour intact.
...
def apply_material_to_parts(name, name_filter=None):
    ...
    and cascade it to their children, so excluding Groups would silently skip
    entire assemblies. Instead every node is tried; only genuine failures
    (cameras, lights, etc.) are skipped, based on what setMaterial() itself
    reports."""
