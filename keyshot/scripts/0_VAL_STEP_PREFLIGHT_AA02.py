...
# with a regular Python interpreter on a folder of *.step/*.stp files, before
# any batch import into KeyShot.
#
# Scale/unit mismatch on import is a common KeyShot import failure mode
# (KeyShot's import dialog has a "Geometry Unit" override for exactly this).
# STEP (ISO 10303-21) is plain text, so a unit mismatch can be caught by
# reading the file as text -- no CAD kernel, no lux API, no KeyShot instance.
#
# Four independent checks against a target folder of STEP files:
#   1. Unit/scale declaration  -- regex-extracts the declared linear unit,
#      flags anything that isn't EXPECTED_UNIT or where none is found.
#   2. Filename hygiene        -- flags characters that would break or
#      surprise a "{baseName}_{view}.{ext}" output-naming convention.
#   3. Duplicate base names    -- case-insensitive collision check ("Bracket.step"
#      and "bracket.STEP" would output the same base name and clobber each other).
...
#      creo_material -> keyshot_template mapping file if one is supplied.
#
# HONESTY NOTE: this is a regex heuristic against ISO-10303-21 text, not a
# real STEP/EXPRESS parser. A real reader resolves entity references across a
# graph (#89 points at #57 points at #12, etc); none of that is attempted
# here. Exporter-specific whitespace/line-wrapping can defeat the regexes --
# when a unit can't be found this says so explicitly rather than assuming
# compliance. The material-hint scan does not walk entity references, so it
# can miss materials one hop away from the matched entity, or report a
# fragment rather than the full string; treat hits as leads, not ground truth.
# The mm SI_UNIT format is confirmed against a real STEP example
# (#89=(LENGTH_UNIT()NAMED_UNIT(*)SI_UNIT(.MILLI.,.METRE.));); everything else
# (CONVERSION_BASED_UNIT inch detection, material-hint scan) is this script's
# own best-effort heuristic, documented as such inline.
#
# Usage:
#   python 0_VAL_STEP_PREFLIGHT_AA02.py /path/to/step/folder [mapping.csv|mapping.json]
# or run with no arguments and it will ask (mapping file is optional).
...
# batch scripts or HOLD_SECONDS/FADE_SECONDS in 3_PRC_FADE_REEL_AA02.py.
# ---------------------------------------------------------------------------
...
def runFilenameHygieneCheck(folder, stepFiles):
    ...
    if badCount:
        print("Downstream output naming is \"{baseName}_{view}.{ext}\" (2a_BAT_STD_VIEW_AA02.py /")
        print("2a_BAT_TURNTABLE_AA02.py) -- spaces, non-ASCII characters, or path separators in a")
...
def runMaterialHintCheck(folder, stepFiles, fileTextCache, mapping):
    ...
    if mapping is not None:
        ...
        print("This is a left-shifted, cheaper version of what 1_HLP_MAT_PREFLIGHT_AA02.py and")
        print("1_HLP_MAT_LOOKUP_AA02.py already check mid-pipeline (both require an actual KeyShot")
        ...
        print("1_HLP_MAT_LOOKUP_AA02.py-format mapping file (.csv or .json) as the second argument")
...
def main():
    ...
    if len(sys.argv) > 2:
        ...
        mappingPath = input(
            ...
            "1_HLP_MAT_LOOKUP_AA02.py) -- press enter to skip: "
        ).strip()
