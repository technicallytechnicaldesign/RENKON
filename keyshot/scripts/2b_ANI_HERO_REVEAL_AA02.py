...
Camera move: track straight in or out to a target camera's own framing,
optional vertical crane, HDRI rotation and brightness ramp synced to the
arrival. Optionally imports the shot object into a named Model Set that
already carries a GUI-authored animation and plays it in sync (no API to
author keyframes from script; only getAnimationFrame/setAnimationFrame/
getAnimationInfo on an existing animation are confirmed). Targets: current
viewport camera, one named camera/Studio, a comma-separated list, or every
camera/Studio in the scene -- one video per target. No orbit/arc: an axis
parallel to the up vector collapses Rodrigues' rotation to zero, so the
move is dolly + vertical crane only (plain vector addition).

Constraints: KeyShot embedded Python < 3.6, ASCII only, no f-strings, no
walrus operator, single-file delivery.

Confirmed: lux.getCameraPosition/LookAt/Up, setCameraPosition/LookAt,
lux.getActiveEnvironment(), env.getBrightness/setBrightness/
setBackplateImage, env.setGroundShadows/setGroundReflections (KeyShot 11.0
scripting reference), lux.renderImage, lux.encodeVideo, lux.openFile(),
lux.getStudios(), lux.getStudio(name) (2024.1+ only), lux.setActiveStudio,
lux.getCameras/setCamera, lux.getModelSets/setModelSets,
lux.getImportOptions, lux.importFile(opts={"model_set_import_to": 1}),
lux.getAnimationFrame/setAnimationFrame, SceneNode.getCenter(world=True),
lux.getRenderOptions/setAddToQueue/setMaxTimeRendering, lux.processQueue.
Unconfirmed: env.setRotation() signature. Experimental:
lux.getAnimationInfo() return shape (dict vs tuple); Matrix.rotate()
signature for the turntable spin.

Run in the KeyShot Scripting Console, or headless via `keyshot -script`
(dialog auto-skips headless -> DEFAULT_OPTIONS used instead).
"""
...
def load_scene_template(path):
    ...
    lux.openFile() opens a file, vs lux.importFile() which brings geometry
    into an existing scene. Non-fatal."""
    if not path:
        return True
    try:
...
def resolve_animation_frame_count():
    ...
    lux.getAnimationInfo() returns duration + frame count, but the exact
    shape (dict vs tuple) isn't documented -- tries a couple of reasonable
    shapes and returns None if none pan out."""
    try:
...
def set_ground_rendering(env, enabled):
    ...
    (isGroundShadowsEnabled-style) and setGroundShadows/setGroundReflections
    are documented; tries a short list of setter name variants per side and
    reports whichever actually worked, for older-build compatibility."""
    if env is None:
        return False
...
# --------------------------------------------------------------------------
# Optional turntable target parts (same approach as the scatter animation)
# --------------------------------------------------------------------------
...
def encode_one_reveal(result, opts, output_folder):
    ...
    exists on disk -- see the queueing note in run_reveal()."""
...
def run_reveal(opts):
    ...
    # Queueing: adds every rendered frame to KeyShot's render queue instead of
    # rendering immediately. A video can only be encoded once every one of its
    # frames has finished rendering, so without process_queue_after this stops
    # after queueing rather than encoding a possibly-incomplete folder.
