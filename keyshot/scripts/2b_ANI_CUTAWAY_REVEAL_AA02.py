...
Sweeps a clipping/section plane through an assembly over a render sequence,
progressively clipping geometry away to expose internal structure; the
camera never moves. Geometry-clipping counterpart to
2b_ANI_HERO_REVEAL_AA02.py (camera moves, never touches clipping) -- kept
as a separate script since the two are different shots with different
failure modes.

Defaults to sweeping the whole loaded scene/model-set bounding box (not a
single part): assemblies expose more interior via a section-plane reveal
than single parts do.

Same many-cameras setup as the hero reveal script: target the current
viewport camera, one named camera/Studio, a comma-separated handful, or
every camera in the scene; one cutaway video is produced per target.

CLIPPING PLANE API: unconfirmed on this KeyShot build. Every clipping-plane
method name (lux.getClippingPlane/getSection/getCuttingPlane/getClipPlane,
addClippingPlane/createClippingPlane/addSection/createSection, and the
handle's setEnabled/enable/setActive/setVisible/setPosition/setOffset/
setOrigin/setPlane/setNormal/setDirection/setAxis) is a naming-convention
guess, tried only via getattr() so a missing name is skipped, not raised.
resolve_clipping_plane() probes getters first, then creators; on the
handle, enable/activate candidates, then per-frame position/normal
candidates. Whichever name works is printed once at setup. If nothing
resolves, the sweep is disabled and frames render un-clipped rather than
raising; the same per-channel disable applies mid-run if a call that
worked at setup starts failing. A documented alternative not implemented
here: a per-node clip attribute (each SceneNode exposing its own
setClippingEnabled()/setClipPlane()) instead of one scene-wide plane --
higher per-frame cost on a large assembly, not wired in.

BOUNDING BOX: SceneNode.getBoundingBox() is confirmed (KeyShot 11.0
scripting reference), returning (min, max) vectors. resolve_bounding_box()
still probes a short candidate list (getBoundingBox(world=True),
getBoundingBox(), getWorldBoundingBox(), getBounds(world=True),
getBounds()) for version tolerance, and accepts a couple of plausible
return shapes (.min/.max object, 2-tuple of points, flat 6-tuple). Falls
back to manual clip_start_override/clip_end_override if none resolve.

STUDIOS vs CAMERAS: a Studio pairs a camera with its environment/lighting
and image style; confirmed lux.getStudios(), lux.getStudio(name),
lux.setActiveStudio(name). A target name matching a Studio activates the
Studio; a plain camera name just switches camera. Camera is never moved
once activated here, unlike the hero reveal script.

GROUND RENDERING: env.setGroundShadows/setGroundReflections are confirmed
setters; set_ground_rendering() tries a short list of setter name variants
defensively.

QUEUEING: "add to queue" queues every rendered frame instead of rendering
immediately. A video can only be encoded once every frame exists, so
without "process queue after" the script queues everything and stops.

CONFIRMED API surface: lux.getActiveEnvironment(), env.getBrightness/
...
SceneNode.getBoundingBox(), env.setGroundShadows/setGroundReflections,
lux.getRenderOptions()/setAddToQueue()/setMaxTimeRendering(),
lux.processQueue(), lux.isHeadless(), lux.getInputDialog().

Inferred-unconfirmed: env.setRotation(). Experimental/unverified: the
entire clipping-plane method set listed above -- treat as needing a real
KeyShot session to validate; update the candidate lists and promote the
working name(s) once confirmed.
...
# --------------------------------------------------------------------------
# getBoundingBox() is confirmed to exist -- see SWEEP GEOMETRY header note.
# Exact return-value shape still probed defensively below.
# --------------------------------------------------------------------------
