...
def build_material(spec):
    ...
    if features["add_scratches"]:
        ...
        if masks.get("mask_scratches_to_edges"):
            mask_bump_layer(graph, scratches_node, add_curvature_mask(graph, edge_radius_mm),
                            "scratches->edges")
            # BENCH 2026-09-02: bump_height cannot be masked on this build (see
            # mask_bump_layer's docstring) -- an unmaskable groove uniform over
            # the WHOLE part defeats the point of asking for edges-only wear, so
            # a masked layer contributes to the (correctly-masked) roughness bus
            # ONLY and is deliberately left OUT of the bump chain.
            print("  [info] scratches->edges: groove depth left OUT of the bump "
                  "chain (unmaskable) -- wear reads as edge-confined roughness "
                  "only, not relief")
        else:
    ...
    if features["add_spots"]:
        ...
        if masks.get("mask_spots_to_cavities"):
            mask_bump_layer(graph, sp, add_occlusion_mask(graph, cavity_radius_mm),
                            "spots->cavities")
            # Same reasoning as scratches->edges above: bump_height is unmaskable,
            # so a masked spots layer skips the bump chain entirely and relies on
            # make_occlusion_color_node's colour tint (added this same bench
            # session) for the crevice-grime read.
            print("  [info] spots->cavities: pit depth left OUT of the bump "
                  "chain (unmaskable) -- grime reads as colour tint only, no "
                  "physical pitting")
        else:
    ...
    if features["add_color_gradient"]:
        # BENCH 2026-09-02: this used to warn about a base.color conflict
        # whenever BOTH checkboxes were on, regardless of whether Color
        # Gradient actually managed to wire anything -- on this build its
        # stops aren't script-settable, so it never wires and the warning was
        # a false alarm every time. Only warn when it actually overwrote the
        # grime wire (add_color_gradient returned True).
        gradient_wired = add_color_gradient(graph, base_node, base_color)
        if gradient_wired and masks.get("mask_spots_to_cavities"):
            print("  [info] colour bus: Color Gradient just overwrote the "
                  "grime tint on base.color -- only one wire survives")
