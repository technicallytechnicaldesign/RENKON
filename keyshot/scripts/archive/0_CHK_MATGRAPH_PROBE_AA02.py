...
# REV AA02
# HEADLESS COMPLIANT
# Material-graph capability probe pack: throwaway diagnostic. Builds one
# disposable scene material ('MATGRAPH_PROBE') and runs 13 probes (MDD-4B7A9F
# section 8), printing PASS/FAIL/MISSING + parameter dumps for each. Does not
# touch scene geometry, only the scratch material (delete it after running).
# Run in the KeyShot Scripting Console; a few probes (labels, displace, film
# sweep) also need a rendered frame to judge by eye -- the script only reports
# whether the graph operation itself succeeded.
#
# KEYSHOT PYTHON CONSTRAINT: no f-strings, ASCII only (embedded interpreter is
# < 3.6 and ASCII-sensitive). Every lux constant is getattr-guarded; every
# probe is wrapped so one failure does not stop the rest.
