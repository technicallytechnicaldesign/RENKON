...
Last updated: 2026-07-16 (bumped the material generator `AB01 → AB02`: root-cause find_param exact-match fix — kills the "noise" vs "Directional Noise" substring collision — plus a Finish preset axis orthogonal to Wear, an explicit Scratches Scale control, and seeded per-material texture placement to kill global tiling repetition; AB01 kept on disk pending recycle. Earlier: retired the NGS scripts — `1_HLP_MAT_GENERATOR` is now the single source of truth for the material generator; made it KeyShot-load-safe + fixed the empty-dialog-default crash; filed the generator into stage 1, synthesised the AUTO/PRO launcher research, added masked-wear + launcher backlog)
...
| 1 | HLP | `1_HLP_MAT_GENERATOR_AB02.py` | 🧪 validating | **Routing fix + Finish axis + anti-repetition** (MDD-4B7A9F / RNK-0054), on top of AB01's spec-refactor (sample→validate→compile, 3 buses, multi-source roughness blending). (1) `find_param` now prefers an EXACT display-name match before the substring scan — fixes the "noise" clobbering "Directional Noise" collision and hardens every setter (Bump Height, Size, Density, Scale…). (2) New **Finish** preset dimension (Pristine/Brushed/Worn/Heavy) orthogonal to Wear — Wear = how much degradation, Finish = character (dir-noise/noise/levels/scratch-groove baseline); default Brushed; plus an explicit Scratches **Scale** control separate from Density. (3) Seeded per-material texture **placement** (offset/rotation/scale-jitter on Scratches/Fine-Noise/Spots/Fractal/Cellular) so builds stop tiling identically — captured in `spec.meta.placement_seed`, reproducible, runs every build. Supersedes AB01 once render-confirmed |
| 1 | HLP | `1_HLP_MAT_GENERATOR_AB01.py` | ⤴️ superseded | Prior candidate — spec-refactor + multi-source roughness blending. Kept on disk until AB02 is render-confirmed, then recycled (operator only) |
...
| P1 | 1 | **Material generator spec-refactor + routing/finish/placement (AB01→AB02)** — MaterialSpec + 3 buses + multi-source roughness blending (Lighten), then the find_param exact-match fix, Finish preset axis + Scratches Scale, and seeded anti-repetition placement. Then families/palettes → finishes → weathering engine → batch "lottery" | 🧪 built, validating | Now `1_HLP_MAT_GENERATOR_AB02.py` (AB01 superseded, kept on disk); render-confirm then retire AA02 + recycle AB01. Later phases build on it, per `MATERIAL_DIVERSITY_DESIGN.md` §9 |
