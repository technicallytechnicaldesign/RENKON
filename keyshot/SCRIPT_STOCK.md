...
| 1 | HLP | `1_HLP_MAT_GENERATOR_AB01.py` | 🧪 validating | **Phase-1 spec-refactor** (MDD-4B7A9F): sample→validate→compile around a MaterialSpec, 3 buses (colour/roughness/bump). New = multi-source roughness blending (scratches+fractal+occlusion via Color Composite/Lighten; fractal/occlusion constrained to a tight band so gloss survives). Emits spec + wire-audit. Preserves every AA02 fix. Supersedes AA02 once render-confirmed |
...
| P1 | 1 | **Material generator spec-refactor (AB01)** — MaterialSpec + 3 buses + multi-source roughness blending (Lighten). Then families/palettes → finishes → weathering engine → batch "lottery" | 🧪 built, validating | Shipped as `1_HLP_MAT_GENERATOR_AB01.py`; render-confirm then retire AA02. Later phases build on it, per `MATERIAL_DIVERSITY_DESIGN.md` §9 |
