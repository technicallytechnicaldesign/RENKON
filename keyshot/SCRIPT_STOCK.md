...
| P1 | 0 | **Material-graph probe pack** `0_CHK_MATGRAPH_PROBE` — one throwaway console script running the 13 probes in `MATERIAL_DIVERSITY_DESIGN.md` §8 (label slot, texture-into-mask, displace, new base/texture node params, image-map paths, blend-mode format, multi-material…) to convert design assumptions into ground truth. Phase 0 of the diversity roadmap | Design | Spec: MDD-4B7A9F §8; needs a human watching renders |
