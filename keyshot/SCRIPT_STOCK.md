...
| P1 | 1 | **Material generator spec-refactor (AB01)** — rebuild AA02 as sample→validate→compile with a MaterialSpec recipe + 3 buses (colour/roughness/bump); lands multi-source roughness blending (Color Composite Lighten/Darken) so scratches+fractal+occlusion coexist. Then families/palettes → finishes → weathering engine → batch "lottery". Full plan: `MATERIAL_DIVERSITY_DESIGN.md` (MDD-4B7A9F) | Design | Phase 1+; do the probe pack first |
