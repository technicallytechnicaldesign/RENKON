...
| P1 | 2a/2b | **Render/animation Phase 1 — load-safety + reliability** (spec: `RENDER_ANIM_REVIEW.md` RAR-6E1F3B §6): (a) f-string→`.format()` + ASCII on the 3 `2b_ANI_*` (they don't load at all) + a regression guard in `4_CHK_AUDIT`; (b) `setActiveStudio`-first version-safe activation in **all 5** (getStudio-before-activate can render **zero images** on KS11 — verified ordering bug); (c) verify-render-wrote + verify-frame-sequence-before-encode; (d) kill the 2a headless `input()` hang; (e) per-part import `try`; (f) env-rotation `%360` | Not started | All confirmed API / pure Python, no probes. Then Phase 2 = L3 shared module + RenderSpec presets + Manifest v2 |
